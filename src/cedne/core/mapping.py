...
class NodeMap(Map):
    ...
    def __init__(self, mapping_dict, **kwargs):
        ...
        self.mapping = (
            len(self.mapping_dict.keys()),
            sum(len(val) for val in self.mapping_dict.values()),
        )
...
class GraphMap(Map):
    ...
    def __init__(self, mapping_dict, graph_1, graph_2, map_type="node", **kwargs):
        ...
        for item_1, item_2 in self.mapping_dict.items():
            if map_type == "node":
                if (isinstance(item_1, str) or isinstance(item_1, Neuron)) and (
                    isinstance(item_2, str) or isinstance(item_2, Neuron)
                ):
                ...
                if (isinstance(item_1, tuple) or isinstance(item_1, Connection)) and (
                    isinstance(item_2, tuple) or isinstance(item_2, Connection)
                ):
