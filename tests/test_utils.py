...
from cedne.utils.loader import makeWorm, getLigandsAndReceptors, make_ciona
...
class TestLoader:
    ...
    def test_makeWorm(self, monkeypatch):
        ...
        monkeypatch.setattr(
            loader_utils, "build_nervous_system", fake_build_nervous_system
        )
        ...
        assert (
            worm.citations["cook_connectome"]
            == loader_utils.citations["cook_connectome"]
        )
    ...
    def test_getLigandsAndReceptors(self, sample_npr_table, sample_ligmap_table):
        ...
        receptor_ligand = getLigandsAndReceptors(
            sample_npr_table, sample_ligmap_table, "AVAL"
        )
    ...
    def test_make_ciona_does_not_fallback_to_nodes_layout(self, tmp_path, monkeypatch):
        ...
        fig1.to_excel(
            tmp_path / "elife-16962-fig1-data1-v1.xlsx",
            sheet_name="Sheet1",
            index=False,
        )
        fig3.to_excel(
            tmp_path / "elife-16962-fig3-data1-v1.xlsx",
            sheet_name="Sheet2",
            index=False,
        )
        ...
        np.testing.assert_array_equal(
            nn.neurons["NeuronA"].position, np.array([10.0, 20.0, 30.0])
        )
...
class TestPlotting:
    ...
    def test_plot_ciona_anatomical_skips_missing_positions(self):
        ...
        with pytest.warns(
            UserWarning, match="Skipping 1 neurons without anatomical coordinates"
        ):
