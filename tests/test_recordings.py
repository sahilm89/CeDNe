...
class TestTrial:
    ...
    def test_recording_property(self, trial, test_signal):
        ...
        assert len(trial.recording) == len(test_signal) - int(
            discard_seconds * F_SAMPLE
        )
    ...
    def test_signal_processing(self, trial):
        ...
        stim_response = StimResponse(
            trial, stim, signal, baseline_samples=baseline_samples
        )
...
class TestStimResponse:
    ...
    def stim_response(self, trial, test_signal, test_stimulus):
        ...
        return StimResponse(
            trial, test_stimulus, test_signal, baseline_samples=baseline_samples
        )
    ...
    def test_feature_extraction(self, stim_response):
        ...
        new_response = StimResponse(
            trial, synthetic_stim, synthetic_response, baseline_samples=baseline_samples
        )
        ...
        assert (
            abs(auc_feature - expected_area) < 0.01
    ...
    def test_response_characteristics(self, stim_response):
        ...
        expected_amplitude = (
            peak_value - baseline_offset
        )  # Peak (1.0) minus baseline (0.1)
        print("\nAmplitude values:")
        ...
        assert (
            abs(characteristics["amplitude"] - expected_amplitude) < 1e-6
        )  # Should be exact since we have no noise
