...
hseq = utils.nx.convert_node_labels_to_integers(
    hseq, first_label=1, ordering="sorted", label_attribute="nodename"
)
...
for ffg in all_ffgs:
    ...
    for med, ned in ffg.items():
        ...
        edgelist += [
            (e[0], e[1], 0) for e in ffg.values() if (e[0], e[1], 0) not in edgelist
        ]
...
for n in nn_chem.neurons:
    ...
    for database, p in jsons.items():
        if database not in optimizable_neurs.keys():
            optimizable_neurs[database] = []
        sortedKeys = sorted([int(x) for x in (p["labeled"].keys())])
        labelledNeurons = {
            p["labeled"][str(x)]["label"]: x
            for x in sortedKeys
            if "?" not in p["labeled"][str(x)]["label"]
        }
        if len(conn_neu_names):
            frac = len(set(labelledNeurons.keys()) & conn_neu_names) / len(
                conn_neu_names
            )
...
def optimizable_neurons(common_set, nn_chem_sub):
    ...
    for n1 in nn_chem_sub.neurons:
        if nn_chem_sub.neurons[n1].type in ["sensory", "interneuron", "motorneuron"]:
            ...
            optimizable = (
                n1 in common_set
            )  # or (any([key[0].name in common_set for key in nn_chem_sub.neurons[n1].incoming().keys()]) and any([key[1].name in common_set for key in nn_chem_sub.neurons[n1].outgoing().keys()]))
            if optimizable:
...
# %%
jsons = {}
for js in os.listdir(
    "/Users/sahilmoza/Documents/Postdoc/Yun Zhang/data/SteveFlavell-NeuroPAL-Cell/Control/"
):
    with open(
        "/Users/sahilmoza/Documents/Postdoc/Yun Zhang/data/SteveFlavell-NeuroPAL-Cell/Control/{}".format(
            js
        ),
        "r",
    ) as f:
...
for datab_in, database in enumerate(jsons.keys()):
    ...
    baselines = {node: 0.0 for node in nn_chem_sub.nodes}
    time_constants = {n: t for n, t in zip(nn_chem_sub.nodes, tconstants)}
    num_timepoints = len(
        jsons[database]["trace_array"][
            measuredNeurons[database][list(measuredNeurons[database].keys())[0]]
        ]
    )
    ...
    for neuron in nn_chem_sub.neurons:
        if neuron in measuredNeurons[database]:
            nn_chem_sub.neurons[neuron].set_property(
                "amplitude",
                jsons[database]["trace_array"][measuredNeurons[database][neuron]],
            )
    ...
    node_parameter_bounds = {
        "gain": {
            rn: gain_base[n.name] * gain_lims
            for n, rn in rate_model.neurons.items()
            if n not in input_nodes
        },
        "time_constant": {
            rn: tconst_base[n.name] * tconst_lims
            for n, rn in rate_model.neurons.items()
            if n not in input_nodes
        },
        "baseline": {
            rn: base_base[n.name] * base_lim
            for n, rn in rate_model.neurons.items()
            if n not in input_nodes
        },
    }
    ...
    edge_parameter_bounds = {
        "weight": {
            (e[0], e[1], e[2]): wts_base[(e[0].name, e[1].name, e[2])] * wts_lim
            for e in rate_model.edges
        }
    }

    real = {
        rate_model.neurons[node]: data["amplitude"]
        for node, data in nn_chem_sub.nodes(data=True)
    ...
    vars_to_fit = [
        rn
        for rn in real.keys()
        if rn not in [rate_model.neurons[n] for n in input_nodes]
    ]
    o = optimizer.OptunaOptimizer(
        rate_model,
        real,
        ...
        study_name=f"{database}_{window_length}_{window_step}_{num_trials}",
    )
...
# %%
f, ax = plt.subplots(
    figsize=(10, 10), nrows=len(jsons.keys()), sharex=True, layout="constrained"
)
...
for database, (pars, mod) in best_models.items():
    for key, val in pars.items():
        ...
        if par not in var_dict:
            var_dict[par] = {}
        if tuple(rest) not in var_dict[par]:
    ...
    for j, k in enumerate(res.keys()):
        ...
        if hasattr(nn_chem_sub.neurons[str(k.name)], "amplitude"):
            ax[j].plot(
                ...
                np.array(nn_chem_sub.neurons[str(k.name)].amplitude),
                color="gray",
            )
            ax[j].set_title(
                f"{np.corrcoef(np.array(nn_chem_sub.neurons[str(k.name)].amplitude)[np.arange(num_timepoints)], res[k])[0,1]}"
            )
        # ax[j].plot(time_points, np.array(nn_chem_sub.neurons[str(k.name)].amplitude)[time_points], label=f'{k.name}-{nn_chem_sub.neurons[str(k.name)].name}', color='gray')
        ax1 = ax[j]
        ax1.plot(
            ...
            res[k],
            color="orange",
            label=f"{k.name}-{nn_chem_sub.neurons[str(k.name)].name}",
        )
...
# %%
f, ax = plt.subplots(
    figsize=(24, 8), nrows=len(var_dict) - 1, layout="constrained", sharex=True
)
...
# %%
f, ax = plt.subplots(
    figsize=(20, 12), layout="constrained", nrows=len(var_dict["weight"]) // 100 + 1
)
xticks = []
...
baselines = {node: 0.0 for node in hseq.nodes}
...
for database in jsons.keys():
    ...
    for j, ffg in enumerate(all_ffgs):
        ...
        for edge in sorted(edges):
            if hasattr(nn_chem_sub.neurons[ffg[edge][0].name], "amplitude") and hasattr(
                nn_chem_sub.neurons[ffg[edge][1].name], "amplitude"
            ):
        ...
        if nodelist:  # and countdown>0:
            if all(n in list(zip(*nodelist))[0] for n in min_motif):
                ...
                for inp in input_nodes:
                    if hasattr(
                        nn_chem_sub.neurons[hseq.nodes[inp]["map"].name], "amplitude"
                    ):
                        input_value = {
                            t: nn_chem_sub.neurons[
                                hseq.nodes[inp]["map"].name
                            ].amplitude[t]
                            for t in time_points
                        }
                        inputs.append(
                            simulator.TimeSeriesInput(input_nodes, input_value)
                        )
                rate_model = simulator.RateModel(
                    hseq,
                    input_nodes,
                ...
                edge_parameter_bounds = {
                    "weight": {e: (-10, 10) for e in rate_model.edges}
                }

                real = {
                    rate_model.neurons[node]: data["map"].amplitude
                    for node, data in hseq.nodes(data=True)
                    if hasattr(data["map"], "amplitude")
                }
                vars_to_fit = [rn for rn in real.keys() if rn not in input_nodes]
                ...
                f, ax = plt.subplots(
                    figsize=(10, 2 * len(hseq.nodes)),
                    nrows=len(hseq.nodes),
                    sharex=True,
                )
                # for k, (n, node) in enumerate(nodelist):
                for j, k in enumerate(best_fit.keys()):
                    if k.name in list(zip(*nodelist))[0]:
                        ax[j].plot(
                            nn_chem_sub.neurons[
                                hseq.nodes[k.name]["map"].name
                            ].amplitude,
                            label=f'{k.name}-{hseq.nodes[k.name]['map'].name}',
                            color="gray",
                        )
                        ax1 = ax[j]
                        ax1.plot(best_fit[k], color="orange")
                        utils.simpleaxis(ax[j])
                        ax[j].set_title(
                            f'{np.corrcoef(nn_chem_sub.neurons[hseq.nodes[k.name]['map'].name].amplitude, best_fit[k])[0,1]}'
                        )
                        ax[j].legend(frameon=False)
                f.suptitle(f"{database}")
                plt.show()
...
rate_model = simulator.RateModel(
    G, input_nodes, weights, gains, time_constants, static_nodes=input_nodes
)
