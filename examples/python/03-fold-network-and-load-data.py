...
# %%
transcripts = pd.read_csv(utils.thres_4, encoding="unicode_escape", index_col=1).drop(
    ["Wormbase_ID", "Unnamed: 0"], axis="columns"
)
...
nodeColors = {
    n: ("orange" if nn.neurons[n].transcript[transcript_name] > 0 else "lightgray")
    for n in connNodes
}
