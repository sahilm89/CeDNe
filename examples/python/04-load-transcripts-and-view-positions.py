...
# %%
utils.plot_position(
    nn, highlight=mec_17, axis="AP-VD", title="mec-17", save="mec-17.svg"
)  # Possible combinations of axis: RL-AP, AP-LR, AP-DV, DV-LR, AP-RL, LR-DV, LR-AP, DV-AP
...
neurons_of_interest = [
    n.name
    for n, t in nn.neurons_have("transcript").items()
    if t[transcript_of_interest]
]

# %%
utils.plot_position(
    nn,
    highlight=neurons_of_interest,
    axis="AP-LR",
    title=transcript_of_interest,
    save=transcript_of_interest + ".pdf",
)
...
neurons_of_interest = [
    ...
    for n, t in nn.neurons_have("transcript").items()
    if all(
        t[transcript_of_interest] for transcript_of_interest in transcripts_of_interest
    )
]
